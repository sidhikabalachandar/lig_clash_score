...
$ $SCHRODINGER/run python3 data.py combine /home/users/sidhikab/lig_clash_score/src/data/run /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/raw --file /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/raw/O38732/2i0a-to-2q5k/grid_no_target_clash_cutoff/0_0_0_poses.csv --index 0
"""
    ...
    if args.task == 'all':
        for file in os.listdir(pose_path):
            if file[-9:] == 'poses.csv':
                ...
                num_poses = 3 * len(df[df['rmsd'] < 2])
                print(num_poses)
                for i in (range(num_poses // 100 + 1)):
                    cmd = 'sbatch -p owners -t 1:00:00 -o {} --wrap="$SCHRODINGER/run python3 data.py group {} {} ' \
                          '--file {} --index {}"'
                    os.system(cmd.format(os.path.join(args.run_path, '{}_{}'.format(file, i)), args.run_path,
                                         args.raw_root, os.path.join(pose_path, file), i))
        ...
        for i in range(args.index * 100, args.index * 100 + 100):
            if i >= 3 * len(df[df['rmsd'] < 2]):
                break
            name = names[i]
            ...
            new_coords = rotate_structure(coords, rot_x, rot_y, rot_z, conformer_center)
            c.setXYZ(new_coords)
            ...
            for clash in steric_clash.clash_iterator(prot_docking, struc2=c):
                ...
                if r not in residues:
                    ...
                    if volume_docking > 5:
                        ...
                        features['bfactor'].append(r.temperature_factor)
        ...
        file_name = args.file.split('/')[-1][:-4]
        out_df.to_csv(os.path.join(save_folder, '{}_{}.csv'.format(file_name, args.index)), index=False)
        print(len(out_df))
        ...
        for file in os.listdir(pose_path):
            if file[-9:] == 'poses.csv':
                ...
                for i in (range(num_poses // 100 + 1)):
                    file_name = file[:-4]
                    if not os.path.exists(os.path.join(save_folder, '{}_{}.csv'.format(file_name, i))):
                        missing.append(os.path.join(save_folder, '{}_{}.csv'.format(file_name, i)))
        ...
        for file in os.listdir(save_folder):
            dfs.append(pd.read_csv(os.path.join(save_folder, file)))

        df = pd.concat(dfs)
        df.to_csv(os.path.join(save_folder, 'combined.csv'))
