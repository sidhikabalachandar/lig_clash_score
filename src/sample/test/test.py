...
def main():
    ...
    if args.task == 'all':
        ...
        for protein, target, start in pairs[:5]:
            ...
            if protein == 'P03368':
                grouped_files = group_files(2, files)
            else:
                grouped_files = group_files(1, files)
        ...
        for file in grouped_files[args.index]:
            ...
            df = pd.read_csv(os.path.join(pose_path, pose_file))
            ...
            # print(len(indices))
            df = df.iloc[indices, :]
            df.to_csv(os.path.join(clash_path, 'pose_pred_data_{}.csv'.format(file_name)))
