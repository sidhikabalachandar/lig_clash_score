...
def parse_ensemble(name, ensemble):
    if ensemble is None:
        ...
        if '_pocket' in name:
            id_code = name.split('_pocket')[-1].split('.mmcif')[0]
            target = name.split('/')[-1].split('_pocket')[0]
        else:
            id_code = name.split('_ligand')[-1].split('.sdf')[0]
            target = name.split('/')[-1].split('_ligand')[0]
        pdb_code = '{}_lig{}'.format(target, id_code)
        df['ensemble'] = pdb_code
    else:
        df = []
