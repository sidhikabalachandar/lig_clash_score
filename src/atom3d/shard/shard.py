...
$ /home/groups/rondror/software/sidhikab/miniconda/envs/geometric/bin/python shard.py all /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/processed /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/sharded/structures@500 --filetype mmcif
$ /home/groups/rondror/software/sidhikab/miniconda/envs/geometric/bin/python shard.py group /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/processed /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/sharded/structures@500 --filetype mmcif
$ /home/groups/rondror/software/sidhikab/miniconda/envs/geometric/bin/python shard.py all /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/processed/3cow /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/sharded/ligands@500 --filetype sdf
"""
...
run_path = '/home/users/sidhikab/lig_clash_score/src/atom3d/shard/run'
...
@click.option('--shard_num', default=-1)
def shard_dataset(task, input_dir, sharded_path, filetype, ensembler, start, stop, shard_num):
    ...
    files = fi.find_files(input_dir, filetype)
    print(len(files))
    ensemble_map = en.ensemblers[ensembler](files)

    if task == 'all':
        Sharded.create_from_ensemble_map(ensemble_map, sharded_path, filetype)
    if task == 'group':
...
class Sharded(object):
    ...
    @classmethod
    def create_from_ensemble_map(cls, ensemble_map, path, filetype):
        ...
        for shard_num in tqdm.trange(num_written, num_shards):
            ...
            cmd = 'sbatch -p owners -t 5:00:00 -o {} --wrap="' \
                  '/home/groups/rondror/software/sidhikab/miniconda/envs/geometric/bin/python shard.py group ' \
                  '/oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/processed ' \
                  '{} --filetype {} --start {} --stop {} --shard_num {}"'
            full_cmd = cmd.format(os.path.join(run_path, 'shard_{}_{}.out'.format(filetype, shard_num)), path, filetype, start, stop, shard_num)
            os.system(full_cmd)
            # print(full_cmd)

    @classmethod
    def shard_one(cls, ensemble_map, path, start, stop, shard_num):
        sharded = cls(path, ['ensemble'])
        dfs = []
        error_count = 0
