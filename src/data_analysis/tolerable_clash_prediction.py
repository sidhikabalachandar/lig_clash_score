...
def main():
    ...
    if args.task == 'all':
        ...
        start_prot = list(structure.StructureReader(os.path.join(pair_path, '{}_prot.mae'.format(start))))[0]
        target_prot = list(structure.StructureReader(os.path.join(pair_path, '{}_prot.mae'.format(target))))[0]
        ...
        for i in range(args.index * 50, args.index * 50 + 50):
            ...
            c.setXYZ(new_coords)
            clash_vol = steric_clash.clash_volume(start_prot, struc2=c)
            if clash_vol > 5:
                residues = {}
                for clash in steric_clash.clash_iterator(target_prot, struc2=c):
                    r = clash[0].getResidue()
                    if r not in residues:
                        clash_vol = steric_clash.clash_volume(target_prot, atoms1=r.getAtomIndices(), struc2=c)
                        residues[r] = clash_vol
