...
ml load chemistry
ml load schrodinger
/home/groups/rondror/software/sidhikab/miniconda/envs/test_env/bin/python decoy_rmsd.py run /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/raw /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/refined_random.txt
/home/groups/rondror/software/sidhikab/miniconda/envs/test_env/bin/python decoy_rmsd.py check /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/raw /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/refined_random.txt
/home/groups/rondror/software/sidhikab/miniconda/envs/test_env/bin/python decoy_rmsd.py MAPK14 /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/raw /oak/stanford/groups/rondror/projects/combind/flexibility/atom3d/refined_random.txt
...
def main():
    ...
    if args.task == 'MAPK14':
        protein = 'MAPK14'
        ligs = ['3D83', '4F9Y']
        with open(os.path.join(run_path, 'rmsd_in.sh'), 'w') as f:
            f.write('#!/bin/bash\n')
            for target in ligs:
                for start in ligs:
                    if target != start:
                        ...
                        f.write('cat {}_prot.mae {} > {}-to-{}_merge_pv.mae\n'.format(start, ' '.join(files), target,
                                                                                      start))
                        f.write(
                            '$SCHRODINGER/run rmsd.py -use_neutral_scaffold -pv second -c {}-to-{}_rmsd.out {}_lig0.mae {}-to-{}_merge_pv.mae\n'.format(
